...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        if(cpp < OSTCBCur->OSTCBPrio){
            ...
            OSPrioCur                   = OSTCBCur->OSTCBPrio;         /* Change current priority            */
        }
        else{
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (OSTCBCur->OSTCBPrio == cpp) {                 /* Did we have to raise current task's priority? */
        ...
        OSPrioCur                   = OSTCBCur->OSTCBPrio;         /* Change the current priority */  
    }
    else{
        sprintf(CtxSwMessage[CtxSwMessageTop++],"%5d  unlock  R%d   (Prio=%d changes to=%d)\n", (int)OSTime, (int)cpp, (int)prio, (int)prio);
    }
