...
void printTCBList(void){
    OS_TCB     *ptcb;
    ptcb = OSTCBList;
    while(ptcb->OSTCBPrio != OS_IDLE_PRIO) {
        sprintf(CtxSwMessage[CtxSwMessageTop++], "---TCBlist time tick %d, prio %d, OSTCBDly %d\n", (int)OSTime, (int)ptcb->OSTCBPrio, (int)ptcb->deadline, (int)ptcb->OSTCBDly);
        ptcb = ptcb->OSTCBNext;
    }
}

INT8U getPrioHighRdy(void){
    OS_TCB     *ptcb;
    INT8U prioHighRdy = OS_IDLE_PRIO;
    INT16U deadline = 100000;
    ptcb = OSTCBList;
    //printTCBList();
    
    // two tasks
    //while(ptcb->OSTCBPrio == 1 || ptcb->OSTCBPrio == 2 || ptcb->OSTCBPrio == 0) {
    // three tasks
